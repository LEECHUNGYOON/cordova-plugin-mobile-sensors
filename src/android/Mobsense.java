package com.neutrinos.senseplugin;
...
public class Mobsense extends CordovaPlugin implements SensorEventListener {
    ...
    public Mobsense() {
        ...
        this.TYPE_SENSOR = "";
        this.timeStamp = 0;
        this.setStatus(Mobsense.STOPPED);
    }
    ...
    public boolean execute(String action, JSONArray args, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("start")) {
            ...
            if(this.start()==Mobsense.STARTING)
            {
            	callbackContext.error("Sensor Started");
                PluginResult r = new PluginResult(PluginResult.Status.OK);
				callbackContext.sendPluginResult(r);
            }
            else
            {
            	callbackContext.error("Failed to start");
            ...
            this.stop();
            if(this.getStatus()==Mobsense.STOPPED)
            {
            	callbackContext.error("Sensor Stopped");
                PluginResult r = new PluginResult(PluginResult.Status.OK);
				callbackContext.sendPluginResult(r);
            }
            else
            {
            	callbackContext.error("Failed to stop");
                PluginResult r = new PluginResult(PluginResult.Status.ERROR);
				callbackContext.sendPluginResult(r);
            }
        }
            ...
            {
                String result = "";
                SensorManager sensorManager = (SensorManager)cordova.getContext().getSystemService(Context.SENSOR_SERVICE);
                ...
                if (listsensor != null)
                {
                    ...
                    {
                        result = result + listsensor.get(i).getName() + "\n";
                    }
                }

                if (result.equals(""))
                {
                    callbackContext.error("No Sensors detected!");
                    PluginResult r = new PluginResult(PluginResult.Status.ERROR);
                    callbackContext.sendPluginResult(r);
                }
                else
            ...
            if (this.status == Mobsense.STOPPED) {
            ...
            else if (this.status != Mobsense.RUNNING) {
                int r = this.start();
                if (r == Mobsense.ERROR_FAILED_TO_START) {
                    callbackContext.sendPluginResult(new PluginResult(PluginResult.Status.IO_EXCEPTION, Mobsense.ERROR_FAILED_TO_START));
                    return true;
                }
    ...
    public int start() {
        ...
        if ((this.status == Mobsense.RUNNING) || (this.status == Mobsense.STARTING)) {
            return this.status;
        }
        ...
        if (list != null && list.size() > 0) {
            ...
            this.setStatus(Mobsense.STARTING);
        } else {
            this.setStatus(Mobsense.ERROR_FAILED_TO_START);
        }
    ...
    private void timeout() {
        if (this.status == Mobsense.STARTING) {
            this.setStatus(Mobsense.ERROR_FAILED_TO_START);
    ...
    public void onSensorChanged(SensorEvent event) {
        try {
            ...
            this.value = value;
            this.setStatus(Mobsense.RUNNING);
